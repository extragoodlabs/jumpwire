...
The JumpWire gateway will automatically generate certificates using [Let's Encrypt](https://letsencrypt.org/), if certificates are **not** loaded into the container and specified by environment variables.

Simply be sure to provide a valid hostname for `JUMPWIRE_DOMAIN`, which our template sets using the Route53 DNS record. The gateway will do the rest when on startup!

#### Self-signed certificate generation

We can provide self-signed certificates that will be used for both TLS and signing the SAML requests. The record generated by Route53 will be `test-db.[route53_zone_name]`

In this example, our Route53 zone name is `hook.jumpwire.cloud`. So we create certificates using:
```shell
...
We update our Dockerfile to copy the certificates into our Docker container, and add ENV variables to configure the gateway to use the keys.
...
We generated our own TLS certificates and provided them to the gateway container. Alternatively, the gateway can generate its own certificates, as long as a valid hostname is provided. Because there is a period of time between when the container task starts and it is reachable, due to health checks, you will need to set `ACME_GENERATE_CERT_DELAY` to a few minutes.

See the full set of ACME parameters in the [README](../README.md)
