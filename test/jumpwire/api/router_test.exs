defmodule JumpWire.API.RouterTest do
  ...
  describe "/manifests/:mid/client-auths" do
    test "GET returns a 200 status with valid SSO" do
      mock_manifest = JumpWire.API.RouterMocks.manifest("test-manifest")
      ...
      # add a client_auth
      conn =
        conn(:post, "/manifests/#{manifest_id}/client-auths", mock_client_auth)
        |> put_auth_header(token)
        |> Router.call(@opts)
      ...
      conn =
        conn(:get, "/manifests/#{manifest_id}/client-auths")
        |> put_auth_header(token)
        |> Router.call(@opts)
    ...
    test "GET /:id returns a 200 status with a valid client_auth ID" do
      ...
      # get the client_auths
      conn =
        conn(:get, "/manifests/#{manifest_id}/client-auths/#{client_auth["id"]}")
        |> put_auth_header(token)
        |> Router.call(@opts)
    ...
    test "DELETE /:id returns a 200 status with valid client_auth ID" do
      ...
      conn =
        conn(:delete, "/manifests/#{manifest_id}/client-auths/#{client_auth["id"]}")
        |> put_auth_header(token)
        |> Router.call(@opts)
